...
class TokenService(
  ...
  fun validateRequestAndGenerateToken(code: UUID?, grantType: String?, redirectUri: URI?, clientId: UUID, refreshToken: String?, authenticationInfo: AuthenticationInfo, nonce:String?): Token {
    val client = clientService.getClientById(clientId).orElseThrow { ApiException(String.format("Client with %s not found", clientId), BAD_REQUEST_CODE) }
    if (code != null && grantType != null && redirectUri != null) {
      ...
      TokenGeneration.validateJwtTokenSignature(refreshToken, secret)
      val claims = TokenGeneration.parseClaims(refreshToken, secret)
      val userId = claims.body["sub"] as String
      val userApprovedClient = userApprovedClientService.getUserApprovedClientByUserIdAndClientId(userId, clientId).orElseThrow {
        throw ApiException(ACCESS_DENIED, ACCESS_DENIED_CODE) }
  ...
  private fun generateToken(prisonerId: String, clientId: UUID, scopes: Set<Scope>, nonce: String?): Token {
    ...
    val idToken = TokenGeneration
      .createToken(idTokenPayload, idTokenService.buildHeaderClaims(SignatureAlgorithm.HS256.name,"JWT"), SignatureAlgorithm.HS256, secret)
    val accessToken = TokenGeneration
      .createToken(accessTokenPayload, accessTokenService.buildHeaderClaims(SignatureAlgorithm.HS256.toString(),"JWT"), SignatureAlgorithm.HS256, secret)
    refreshTokenPayload["ati"] = accessTokenPayload["jti"] as String
    val refreshToken = TokenGeneration
      .createToken(refreshTokenPayload, accessTokenService.buildHeaderClaims(SignatureAlgorithm.HS256.toString(),"JWT"), SignatureAlgorithm.HS256, secret)
